// SPDX-FileCopyrightText: 2026 Darling Team
// SPDX-License-Identifier: MIT-0

#include <darling-directsyscall/impl/bsd_cbridge_syscall.h>

#include <darling-directsyscall/args.h>
#include <darling-directsyscall/syscall.h>
#include <darling-directsyscall/sysnum.h>

off_t sys_lseek(int fd, off_t offset, int whence, int* errno) {
    syscall_return_args_t retargs;
    int sysnum;
    
#if defined(__x86_64__)
    sysnum = X86_64_SYSCALL_CLASS_UNIX(SYSNUM_LSEEK);
#else
    sysnum = SYSNUM_LSEEK;
#endif

#if defined(__i386__)
    uint32_t offset_lower;
    uint32_t offset_upper;
    split_args_arch32_off_t(offset, &offset_lower, &offset_upper);
#endif

#if defined(__i386__)
    CALL_SYSENTER_SYSCALL_ARG4(sysnum, &retargs, fd, offset_lower, offset_upper, whence);
#else // default
    CALL_GENERIC_SYSCALL_ARG3(sysnum, &retargs, fd, offset, whence);
#endif

    return return_off_t(&retargs, errno);
}
